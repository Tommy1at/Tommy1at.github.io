---
layout: post
title: "Willkommen auf Tommys ProjektSeite"
date: 2026-09-10 07:00:00 +0200
description: Ausgewaehlte Projekte aus Webentwicklung, Technik und Vereinsarbeit.
img: aboutme-2-2.webp
pinned: true
image_fit: contain
image_alt: Dominik C. Aspeck
---

Auf dieser Seite stelle ich ausgewaehlte private, unternehmerische und vereinsbezogene Projekte vor. Die Beitraege geben Einblicke in die Umsetzung, die eingesetzten Technologien und die Weiterentwicklung.

Ein Klick auf mein Foto (links) fuehrt zur Startseite zurueck.

Die Webseite verwendet Markdown und ein Jekyll-Theme. Die Inhalte werden mit Git versioniert und ueber GitHub Pages veroeffentlicht.

> Technologien: Markdown, Jekyll, Git, GitHub Actions, GitHub Pages.

## Weiterentwicklung – September 2026

* Die beiden identischen Willkommen-Beitraege in dieser Fassung zusammengefuehrt.
* Die englischen Beispieltexte und das Ruby-Demobeispiel durch eine kurze Einfuehrung zur Projektseite ersetzt.

## Stand vom 04. Juni 2024

* Die Projektseite stellte ausgewaehlte Projekte vor.
* Die Navigation zur Startseite erfolgte ueber das Autorenfoto.
* Der Willkommen-Beitrag enthielt noch ein Ruby-Codebeispiel und englische Jekyll-Hinweise aus der Vorlage.
